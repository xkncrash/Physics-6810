...
//      04/25/22  modified for use with grav_test.cpp
//
//*****************************************************************
...
class GnuplotPipe
{
 public:
  ...
  void set_ratio (const bool ratio) {square_ratio = ratio;};
  void set_delay (const int t_delay) {delay = t_delay;};
  int get_N () {return N;};

  int init (double *M);                // initialize a plot
  int plot (const double x, const double y, const int n_count);  // plot a point on first line
  //int plot2 ();  // plot a point on second line
 ...
 private:
  ...
  bool square_ratio;  // signals a square ratio
